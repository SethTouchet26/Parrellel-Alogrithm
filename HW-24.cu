...
void setup()
{
    ...
    for (int d = 0; d < 2; d++)	
	{
		...
		cudaMalloc(&MGPU[d],N*sizeof(float));
		cudaErrorCheck(__FILE__, __LINE__);
		cudaMalloc(&PGPU[d],N*sizeof(float3));
		cudaErrorCheck(__FILE__, __LINE__);
		cudaMalloc(&VGPU[d],N*sizeof(float3));
		cudaErrorCheck(__FILE__, __LINE__);
...
void nBody()
{
	...
	while(t < RUN_TIME)
	{
		for(int d = 0; d < 2; d++)
		{
			...
			getForces<<<GridSize,BlockSize>>>(PGPU[0], VGPU[0], FGPU[0], MGPU[0], G, H, N);
			cudaErrorCheck(__FILE__, __LINE__);
			moveBodies<<<GridSize,BlockSize>>>(PGPU[0], VGPU[0], FGPU[0], MGPU[0], Damp, dt, t, N);
			cudaErrorCheck(__FILE__, __LINE__);
		}
